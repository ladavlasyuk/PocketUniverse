...
extension Notification.Name {
    static let portalConnectionFailed = Notification.Name("portalConnectionFailed")
}
...
final class ContentPortalController: UIViewController {
    ...
    private func setupContentView() {
        ...
        if let overlay = loadingOverlay {
            view.insertSubview(contentView, belowSubview: overlay)
        } else {
    ...
    private func loadDestination() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 15) { [weak self] in
            ...
            if !self.hasLoadedMainContent && self.contentView.url == nil {
                print("[Portal] Load timed out with no content — showing no internet")
    ...
    func reload(address: URL) {
        ...
        setupContentView()
        destination = address.absoluteString
        navigationCoordinator?.lastNavigatedAddress = address
        hideErrorView()
        hasLoadedMainContent = false
    ...
    func handleMainFrameLoadFailure(_ error: NSError) {
        ...
        print("[Portal] Main-frame load failed again (\(error.code)) — showing no internet")
        reportConnectionFailure()
    }

    private func reportConnectionFailure() {
        NotificationCenter.default.post(name: .portalConnectionFailed, object: nil)
    }
