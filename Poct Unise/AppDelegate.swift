...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func persistSystem(_ snapshot: SessionSnapshot) {
        SystemStore.shared.save(snapshot)
    }

    func fetchSystems() -> [SavedSystem] {
    ...
    private var didFinishCoreLaunchSetup = false
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        continueLaunchFlow()
        didFinishCoreLaunchSetup = true
        return true
    }

    func attachWindow(_ newWindow: UIWindow, launchOptions: UIScene.ConnectionOptions?) {
        window = newWindow
        if let remoteNotification = launchOptions?.notificationResponse?.notification.request.content.userInfo {
            handleLaunchPushNotification(remoteNotification)
        }
        setupWindow()
        if LaunchState.shared.isPermanentNativeFlow() {
        ...
        if !didFinishCoreLaunchSetup {
            continueLaunchFlow()
            didFinishCoreLaunchSetup = true
        }
        if LaunchState.shared.portalDestination != nil
            || LaunchState.shared.didOpenPushDestination
            || didReceiveConfigResponse {
            DispatchQueue.main.async { [weak self] in
                self?.updateLaunchPresentation()
            }
        }
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse
                ...
                if LaunchState.shared.openLastSuccessfulDestinationIfAvailable() {
                    self.logTiming("No server response within timeout — opening last successful destination")
                    self.didReceiveConfigResponse = true
                    self.expectsPortalDestination = true
                } else {
    ...
    private func updateLaunchPresentation() {
        ...
        if state.noInternetMessage != nil {
            showNoInternetScreen()
        } else if let destination = state.portalDestination {
            OrientationController.shared.unlockAllOrientations()
    ...
    private func observeLaunchState() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePortalConnectionFailure),
            name: .portalConnectionFailed,
            object: nil
        )
    }
    ...
    @objc private func handlePortalConnectionFailure() {
    ...
    private func continueLaunchFlow() {
        ...
        if LaunchState.shared.portalDestination != nil || LaunchState.shared.didOpenPushDestination {
            print("[AFSDK] Portal destination already opened by push. Skipping launch flow.")
            return
        }

        if LaunchState.shared.activateStoredDestinationIfValid() {
            OrientationController.shared.unlockAllOrientations()
            print("[AFSDK] Stored portal destination is valid. Opening it without a config request.")
            didReceiveConfigResponse = true
            return
        }

        if LaunchState.shared.lastSuccessfulDestination() != nil {
            print("[AFSDK] Stored portal destination expired or pending refresh. Re-requesting config, last URL kept as fallback.")
        }

        if let storedPayload = LaunchState.shared.storedConfigPayload() {
            print("[AFSDK] Reusing saved payload for config request.")
    ...
    private func handleRemoteNotificationUserInfo(_ userInfo: [AnyHashable: Any], fromUserTap: Bool) {
        ...
        guard !LaunchState.shared.isPermanentNativeFlow() else {
            print("[Push] Tap ignored — permanent native flow is active")
            return
        }

        let addressString = extractPushAddress(from: userInfo)
        DispatchQueue.main.async {
            if let addressString {
                print("[Push] Opening push URL: \(addressString)")
                _ = LaunchState.shared.openSessionPushDestination(addressString)
            } else {
                print("[Push] Tap has no url — opening stored config destination")
                _ = LaunchState.shared.openStoredConfigDestination()
            }
        }
    }
    ...
    private func handleLaunchPushNotification(_ userInfo: [AnyHashable: Any]) {
        ...
        guard let addressString = extractPushAddress(from: userInfo) else {
            print("[Push] Cold launch push has no url — launch will use the config destination")
            return
        }

        print("[Push] Cold launch push url kept in memory for this launch: \(addressString)")
        _ = LaunchState.shared.openSessionPushDestination(addressString)
    }
    ...
    private func sendMergedPayload(_ payload: [String: Any], applyDestinationToCurrentSession: Bool = true) {
        if !LaunchState.shared.isStoredDestinationExpired() {
            print("[AFSDK] Stored portal destination is still valid. Skipping config request.")
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    self.finishConfigRequestWithoutConnection(
                ...
                guard let httpResponse = response as? HTTPURLResponse else {
                    print("[AFSDK] Config server did not return an HTTP response.")
                    self.finishConfigRequestWithoutConnection(
                        applyDestinationToCurrentSession: applyDestinationToCurrentSession
                    )
                    return
                ...
                LaunchState.shared.clearNoInternetMessage()

                guard (200...299).contains(status) else {
                    print("[AFSDK] Server responded without a link. Native flow will open.")
                    guard applyDestinationToCurrentSession else { return }
                    self.didReceiveConfigResponse = true
                    self.expectsPortalDestination = false
                    self.updateLaunchPresentation()
                    return
                }
    ...
    private func finishConfigRequestWithoutConnection(applyDestinationToCurrentSession: Bool) {
        guard applyDestinationToCurrentSession else {
            print("[AFSDK] Background config refresh failed — launch UI unchanged.")
            return
        }
        guard LaunchState.shared.portalDestination == nil, !LaunchState.shared.didOpenPushDestination else {
            print("[AFSDK] Config connection failed — keeping the page already open.")
            return
        }
        if LaunchState.shared.openLastSuccessfulDestinationIfAvailable() {
            print("[AFSDK] Config server did not connect. Opening last successful destination.")
            didReceiveConfigResponse = true
            expectsPortalDestination = true
            updateLaunchPresentation()
            return
        }
        print("[AFSDK] Config server did not connect and no saved destination exists. Showing no internet.")
        LaunchState.shared.showNoInternetMessage()
        didReceiveConfigResponse = true
        expectsPortalDestination = false
        updateLaunchPresentation()
    ...
    private func handleMergedPayloadResponse(
        ...
        guard !LaunchState.shared.isPermanentNativeFlow() else {
            print("[AFSDK] Permanent native flow is active. Ignoring portal destination from server response.")
            return false
        }
...
extension AppDelegate: MessagingDelegate {
    ...
    private func resendConfigWithUpdatedPushTokenIfNeeded(_ token: String) {
        ...
        if !LaunchState.shared.isStoredDestinationExpired() {
            print("[Push] Stored portal destination is still valid. Skipping config resend.")
            return
        }
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        NotificationHandler.shared.removeDeliveredNotification(identifier: notificationId)
