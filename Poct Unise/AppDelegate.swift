...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func continueLaunchFlow() {
        ...
        if LaunchState.shared.activateStoredDestinationIfValid() {
            OrientationController.shared.unlockAllOrientations()
            print("[AFSDK] Stored portal destination is valid. Portal flow will open.")
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    self.finishConfigRequestWithoutFreshDestination(
    ...
    private func finishConfigRequestWithoutFreshDestination(
        ...
        if LaunchState.shared.portalDestination == nil,
           LaunchState.shared.openLastSuccessfulDestinationIfAvailable() {
            print("[AFSDK] Config unavailable — opened last successful destination.")
            didReceiveConfigResponse = true
        ...
        if let networkError {
            print("[AFSDK] Config unavailable: \(networkError.localizedDescription)")
        } else {
            print("[AFSDK] Config unavailable after failed attempts.")
        }
    ...
    private func handleMergedPayloadResponse(
        ...
        guard let data = data,
              let json = try? JSONSerialization.jsonObject(with: data),
              let payload = json as? [String: Any] else {
            print("[AFSDK] Server response is empty or invalid.")
        ...
        guard let destination = destination else {
            print("[AFSDK] Server returned ok=true without a destination.")
            if applyDestinationToCurrentSession,
               LaunchState.shared.openLastSuccessfulDestinationIfAvailable() {
                return true
            }
