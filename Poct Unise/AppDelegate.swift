...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            self.logTiming("Launch loader SAFETY timeout reached (didReceiveConfigResponse=\(self.didReceiveConfigResponse), configInFlight=\(self.isConfigRequestInFlight))")

            if self.isConfigRequestInFlight {
                self.logTiming("Config request still in flight — keeping loader")
                return
            }
    ...
    private func continueLaunchFlow() {
        ...
        if let storedPayload = LaunchState.shared.storedConfigPayload() {
            print("[AFSDK] Stored portal destination expired. Reusing saved payload for config request.")
    ...
    private func handleRemoteNotificationUserInfo(_ userInfo: [AnyHashable: Any], fromUserTap: Bool) {
        ...
        if isLaunchLoaderActive && LaunchState.shared.portalDestination == nil {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.presentPortalDestination(addressString)
        }
    }
    ...
    private func sendMergedPayload(
        ...
        logTiming("Config POST sent to \(AppConstants.configEndpoint) (attempt \(attempt + 1)/\(totalAttempts), timeout=\(Int(timeout))s)")
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            DispatchQueue.main.async {
                guard let self else { return }
                self.logTiming("Config POST response received (attempt \(attempt + 1)/\(totalAttempts))")
    ...
    private func retryOrFinishConfigRequest(
        ...
        if attempt + 1 < totalAttempts {
            ...
            logTiming("Config POST retry \(attempt + 2)/\(totalAttempts) (timeout=\(Int(nextTimeout))s)")
            sendMergedPayload(
                payload,
                applyDestinationToCurrentSession: applyDestinationToCurrentSession,
                attempt: attempt + 1
    ...
    private func finishConfigRetriesWithoutResponse(applyDestinationToCurrentSession: Bool) {
        ...
        print("[AFSDK] Config POST failed after 15s + 15s + 30s.")
